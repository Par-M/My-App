...
class SchedulingService:
    ...
    def generate(self, request: ScheduleGenerateRequest) -> AIRecommendation:
        ...
        if result is None:
            return self._store_failed(context, failure)
    ...
    def _store_pending(self, context, *, reasoning, items, meta) -> AIRecommendation:
    ...
    def _store_failed(self, context, failure: str | None) -> AIRecommendation:
        recommendation = AIRecommendation(
            user_id=self.user_id,
            status=RecommendationStatus.pending,
            accepted=False,
    ...
    def accept(
        ...
        items = (recommendation.recommendation or {}).get("items", [])
        if not items:
            raise RecommendationNotAcceptableError(
